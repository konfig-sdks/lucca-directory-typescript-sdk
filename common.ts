...
import { readableStreamToString, LuccaDirectoryError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new LuccaDirectoryError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new LuccaDirectoryError(e, e.response?.data)
                }
                if (innerError instanceof LuccaDirectoryError) {
                    // Got "LuccaDirectoryError" from the above try block
                    throw innerError;
                }
